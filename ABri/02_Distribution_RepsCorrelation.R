...
p_hist<-ggplot(dist_singles, aes(x=factor(type, levels=c("Synonymous", "Nonsense", "Missense")), y=nscore_c))+
  ...
  scale_y_continuous(name="Nucleation Score", breaks=c(-8, -4, 0, 4, 8), labels=c(-8, -4, 0, 4, 8), limits=c(-8, 8))
p_hist
...
p_corr_12<-ggplot(singles_stops, aes(x=nscore1_c, y=nscore2_c) )+
  stat_binhex()+
  theme_bw()+
  annotate("text", x = 5, y = -1.2, label = paste0("R=", round(R, 2)), size=5)+
  annotate("text", x = 5, y = -2, label = paste0("p=",format(p, digits = 2, scientific = T)), size=5)+
  ...
  scale_x_continuous(name="Replicate 1", breaks=c(-2, 0, 2, 4, 6), labels=c(-2, 0, 2, 4, 6), limits=c(-2, 6))+
  scale_y_continuous(name="Replicate 2", breaks=c(-2, 0, 2, 4, 6), labels=c(-2, 0, 2, 4, 6), limits=c(-2, 6))
...
p_corr_13<-ggplot(singles_stops, aes(x=nscore1_c, y=nscore3_c) )+
  ...
  geom_smooth(method = "lm", linetype = 2, size=1, se = F, color = "black")+
  scale_x_continuous(name="Replicate 1", breaks=c(-2, 0, 2, 4, 6), labels=c(-2, 0, 2, 4, 6), limits=c(-2, 6))+
  scale_y_continuous(name="Replicate 3", breaks=c(-2, 0, 2, 4, 6), labels=c(-2, 0, 2, 4, 6), limits=c(-2, 6))
...
p_corr_23<-ggplot(singles_stops, aes(x=nscore2_c, y=nscore3_c) )+
  ...
  scale_fill_gradient(high="grey30", low="grey90")+
  geom_smooth(method = "lm", linetype = 2, size=1, se = F, color = "black")+
  scale_x_continuous(name="Replicate 2", breaks=c(-2, 0, 2, 4, 6), labels=c(-2, 0, 2, 4, 6), limits=c(-2, 6))+
  scale_y_continuous(name="Replicate 3", breaks=c(-2, 0, 2, 4, 6), labels=c(-2, 0, 2, 4, 6), limits=c(-2, 6))
...
individual_validation<-read_tsv("ABri_IndividualValidation.tsv")

individual_validation<-individual_validation %>% group_by(ID) %>% summarise(mean_growth=mean(growth_rate), 
                                                                            std_growth=sd(growth_rate))

# ABri individual validation: mean growth rate = 0.006, std = 0.004

individual_validation<-left_join(individual_validation, singles_stops[c("ID", "nscore_c", "sigma")], by="ID")

ABri<-data.frame("ID"="ABri", "mean_growth" = 0.006, "std_growth" = 0.004, "nscore_c"=-0.5565460, "sigma"=0.2522805)

individual_validation<-rbind(individual_validation, ABri)

individual_validation<-individual_validation[individual_validation$ID != "L-20-C",]
...
p_small_large_scale<-ggplot(individual_validation, aes(x=mean_growth, y=nscore_c))+
  ...
  geom_point(size=2)+
  annotate("text", x = 4.8, y = -1.2, label=paste0("R=",round(R, 2)), size=5)+
  annotate("text", x = 4.8, y = -2, label =paste0("p=",format(p, digits = 2, scientific = T)), size=5)+
  scale_y_continuous(breaks=c(-2, 0, 2, 4, 6), labels=c(-2, 0, 2, 4, 6), limits=c(-2, 6))+
  geom_label_repel(aes(label=ID), seed=42, box.padding = 0.8)+
  theme_bw()+
