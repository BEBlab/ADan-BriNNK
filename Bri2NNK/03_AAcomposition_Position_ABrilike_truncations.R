...
# This subset have few truncations. 
# Let's see how many of the shorter ones nucleate and what are the features describing them.

# Length of extension:
seq_len<-3

NNK_all_df<-NNK_all_df[NNK_all_df$aa_len == seq_len,]
print(nrow(NNK_all_df))

#
dir.create(paste0("03_AAcomposition_Position_Bri2nterm_", seq_len))
path=paste0("03_AAcomposition_Position_Bri2nterm_", seq_len)
...
aa_frequencies_pos_all<-rbind(pos_2, pos_3)
...
freq_heatmap_all<-ggplot(co_all, aes(factor(Var2, labels=(24:(22+seq_len))), factor(Var1, levels=rev(vectorAA))))+
...
#
p_scatter_all<-ggplot(co_all, aes(x=factor(Var2, labels=24:(22+seq_len)), y=value, group=1))+
  geom_point()+
  geom_line()+
  ylim(c(-0.15, 0.15))+
...
aa_frequencies_pos_nucleators_<-rbind(pos_2, pos_3)
...
freq_heatmap_nucleators<-ggplot(co_nucleators, aes(factor(Var2, labels=(24:(22+seq_len))), factor(Var1, levels=rev(vectorAA))))+
  geom_tile(aes(fill=value), color="white", size=1)+
...
#
p_scatter_nucleators<-ggplot(co_nucleators, aes(x=factor(Var2, labels=24:(22+seq_len)), y=value, group=1))+
...
aa_frequencies_pos_nonnucleators_<-rbind(pos_2, pos_3)
...
freq_heatmap_nonnucleators<-ggplot(co_nonnucleators, aes(factor(Var2, labels=(24:(22+seq_len))), factor(Var1, levels=rev(vectorAA))))+
  geom_tile(aes(fill=value), color="white", size=1)+
...
#
p_scatter_nonnucleators<-ggplot(co_nonnucleators, aes(x=factor(Var2, labels=24:(22+seq_len)), y=value, group=1))+
...
diff_heatmap_nucleators_nonnucleators<-ggplot(co_nucleators_nonnucleators, aes(factor(Var2, labels=(24:(22+seq_len))), factor(Var1, levels=rev(vectorAA))))+
...
p_violin_p<-ggplot(co_nucleators_nonnucleators, aes(x=factor(Var2, labels=(24:(22+seq_len))), y=diff))+
...
p_scatter_nucleators_nonnucleators<-ggplot(co_nucleators_nonnucleators, aes(x=factor(Var2, labels=24:(22+seq_len)), y=freq ,group=group, color=group))+
  geom_point(size=2)+
  geom_text(aes(label=significance), size=5, color="black", vjust=0.65, angle=90, y=0.47)+
  geom_line(size=1)+
...
p_scatter_nucleators_nonnucleators_types<-ggplot(co_nucleators_nonnucleators_sum, aes(x=factor(Var2, labels=24:(22+seq_len)), y=sum_freq ,group=group, color=group))+
  geom_point(size=2)+
  geom_line(size=1)+
  geom_text(aes(label=significance), size=5, color="black", vjust=0.65, angle=90, y=0.37)+
  scale_color_manual(values=c("darkred", "darkblue"), label=c("non-nucleator", "nucleator"))+
  ylim(c(-0.4, 0.4))+
