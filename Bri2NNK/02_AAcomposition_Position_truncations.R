...
### AA
#required data:
load(paste0("freq_12.RData"))
co_nucleators_nonnucleators$aa_len<-12
...
p_scatter_nucleators_nonnucleators<-ggplot(co_nucleators_nonnucleators_all, aes(x=factor(Var2, labels=(23:34)), y=freq ,group=group, color=group))+
  geom_point(size=2)+
  geom_text(aes(label=significance), size=5, color="black", vjust=0.65, angle=90, y=0.12)+
  geom_line(size=1)+
  scale_color_manual(values=c("darkred", "darkblue"), label=c("non-nucleator", "nucleator"))+
  ylim(c(-0.2, 0.2))+
  facet_grid(factor(aa_len, labels=(24:34))~factor(Var1, levels=vectorAA))+
  labs(x="Position", y="Normalized Frequency")+
  scale_x_discrete(breaks=c(24,26,28,30,32,34))+
  theme_bw()+
  theme(
    #plot.margin = ggplot2::unit(c(1, 1, 1, 1), "points"),
...
#required data:
load(paste0("freq_12.RData"))
co_nucleators_nonnucleators_sum$aa_len<-12
...
p_scatter_nucleators_nonnucleators_types<-ggplot(co_nucleators_nonnucleators_all, aes(x=factor(Var2, labels=(23:34)), y=sum_freq ,group=group, color=group))+
  geom_point(size=2)+
  geom_line(size=1)+
  geom_text(aes(label=significance), size=5, color="black", vjust=0.65, angle=90, y=0.19)+
  scale_color_manual(values=c("darkred", "darkblue"), label=c("non-nucleator", "nucleator"))+
  ylim(c(-0.21, 0.21))+
  facet_grid(factor(aa_len, labels=(24:34))~factor(type, levels=c("glycine", "proline", "cysteine", "positive", "negative", "polar", "aliphatic", "aromatic")))+  
  labs(x="Position", y="Normalized Frequency", color="")+
  scale_x_discrete(breaks=c(24,26,28,30,32,34))+
  coord_cartesian(clip = "off")+
  theme_bw()+
  annotate("segment", x=-Inf, xend=Inf, y=-Inf, yend=-Inf)+
...
p_scatter_nucleators_nonnucleators_types<-ggplot(co_nucleators_nonnucleators_all[co_nucleators_nonnucleators_all$type == "aromatic",], 
                                                 aes(x=factor(Var2, labels=(23:34)), y=sum_freq ,group=group, color=group))+
  geom_point(size=2)+
  geom_line(size=1)+
  geom_text(aes(label=significance), size=5, color="black", vjust=0.65, angle=90, y=0.24)+
  scale_color_manual(values=c("darkred", "darkblue"), label=c("non-nucleator", "nucleator"))+
  ylim(c(-0.27, 0.27))+
  facet_wrap(~factor(aa_len, labels=(24:34)), nrow=2)+  
  labs(x="Position", y="Normalized Frequency", title="Aromatics", color="")+
  scale_x_discrete(breaks=c(24,26,28,30,32,34))+
  coord_cartesian(clip = "off")+
  theme_bw()+
  annotate("segment", x=-Inf, xend=Inf, y=-Inf, yend=-Inf)+
