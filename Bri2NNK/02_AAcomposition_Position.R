...
dir.create("02_AAcomposition_Position")
path="02_AAcomposition_Position"
...
NNK_all_df$pos_12<-sapply(strsplit(NNK_all_df$aa_seq_cor, ""), '[', 12)
...
pos_12<-table(nucl_df$pos_12)/length(nucl_df$pos_12)
for (aa in vectorAA){
  if (!(aa %in% names(pos_12))){
    pos_12[aa]<-0}}
pos_12<-pos_12[order(names(pos_12))]


aa_frequencies_pos_all<-rbind(pos_1, pos_2, pos_3, pos_4, pos_5, pos_6,
...
freq_heatmap_all<-ggplot(co_all, aes(factor(Var2, labels=(23:34)), factor(Var1, levels=rev(vectorAA))))+
...
#
p_scatter_all<-ggplot(co_all, aes(x=factor(Var2, labels=23:34), y=value, group=1))+
  geom_point()+
  geom_line()+
  ylim(c(-0.15, 0.15))+
...
nucleators_nucleators$pos_12<-sapply(strsplit(nucleators_nucleators$aa_seq_cor, ""), '[', 12)
...
aa_frequencies_pos_nucleators_<-rbind(pos_1, pos_2, pos_3, pos_4, pos_5, pos_6,
                                      pos_7, pos_8, pos_9, pos_10, pos_11, pos_12)
...
freq_heatmap_nucleators<-ggplot(co_nucleators, aes(factor(Var2, labels=(23:34)), factor(Var1, levels=rev(vectorAA))))+
  geom_tile(aes(fill=value), color="white", size=1)+
...
#
p_scatter_nucleators<-ggplot(co_nucleators, aes(x=factor(Var2, labels=23:34), y=value, group=1))+
...
nucleatorsTop10<-NNK_all_df[NNK_all_df$category_fdr == "Top10 nucleators",]

nucleatorsTop10$pos_1<-sapply(strsplit(nucleatorsTop10$aa_seq_cor, ""), '[', 1)
nucleatorsTop10$pos_2<-sapply(strsplit(nucleatorsTop10$aa_seq_cor, ""), '[', 2)
nucleatorsTop10$pos_3<-sapply(strsplit(nucleatorsTop10$aa_seq_cor, ""), '[', 3)
nucleatorsTop10$pos_4<-sapply(strsplit(nucleatorsTop10$aa_seq_cor, ""), '[', 4)
nucleatorsTop10$pos_5<-sapply(strsplit(nucleatorsTop10$aa_seq_cor, ""), '[', 5)
nucleatorsTop10$pos_6<-sapply(strsplit(nucleatorsTop10$aa_seq_cor, ""), '[', 6)
nucleatorsTop10$pos_7<-sapply(strsplit(nucleatorsTop10$aa_seq_cor, ""), '[', 7)
nucleatorsTop10$pos_8<-sapply(strsplit(nucleatorsTop10$aa_seq_cor, ""), '[', 8)
nucleatorsTop10$pos_9<-sapply(strsplit(nucleatorsTop10$aa_seq_cor, ""), '[', 9)
nucleatorsTop10$pos_10<-sapply(strsplit(nucleatorsTop10$aa_seq_cor, ""), '[', 10)
nucleatorsTop10$pos_11<-sapply(strsplit(nucleatorsTop10$aa_seq_cor, ""), '[', 11)
nucleatorsTop10$pos_12<-sapply(strsplit(nucleatorsTop10$aa_seq_cor, ""), '[', 12)


nucl_df<-nucleatorsTop10
...
aa_frequencies_posTop10_<-rbind(pos_1, pos_2, pos_3, pos_4, pos_5, pos_6,
                                pos_7, pos_8, pos_9, pos_10, pos_11, pos_12)
aa_frequencies_posTop10_<-as.data.frame(aa_frequencies_posTop10_)

# not normalized
aa_frequencies_posTop10<-t(aa_frequencies_posTop10_)

coTop10=melt(aa_frequencies_posTop10)
coTop10<-coTop10[coTop10$Var1 != "*",]

freq_heatmapTop10<-ggplot(coTop10, aes(factor(Var2, labels=(23:34)), factor(Var1, levels=rev(vectorAA))))+
  geom_tile(aes(fill=value), color="white", size=1)+
  scale_fill_gradient2(high="darkviolet", mid= "white", low="black", limit=c(0, 0.25))+
  labs(x="Position", y="Amino acid", fill="Frequency", title="AA frequencies per position in Top10 nucleators")+
  theme_minimal()+
  theme(
    plot.margin = ggplot2::unit(c(1, 1, 1, 1), "points"))
print(freq_heatmapTop10)

ggsave(freq_heatmapTop10, file="AA_frequencyTop10.jpg", width = 14, height = 8, path=path)

#
p_scatterTop10<-ggplot(coTop10, aes(x=factor(Var2, labels=23:34), y=value, group=1))+
  geom_point(color="darkviolet")+
  geom_line(color="darkviolet")+
  ylim(c(-0.25, 0.25))+
  facet_wrap(~factor(Var1, levels=vectorAA))+
  labs(x="Position", y="Frequency", title="Top10 nucleators")+
  theme_bw()+
  theme(
    plot.margin = ggplot2::unit(c(1, 1, 1, 1), "points"),
    legend.position = c(0.85, 0.2),
...
ggsave(p_scatterTop10, file="AA_frequencyTop10_pos.jpg", width = 14, height = 8, path=path)


# Normalized

aa_frequencies_posTop10<-t(aa_frequencies_posTop10_)-aa_frequencies_pos_all
...
freq_heatmapTop10<-ggplot(coTop10, aes(factor(Var2, labels=(23:34)), factor(Var1, levels=rev(vectorAA))))+
  geom_tile(aes(fill=value), color="white", size=1)+
  scale_fill_gradient2(high="darkviolet", mid= "white", low="black", limit=c(-0.25, 0.25))+
  labs(x="Position", y="Amino acid", fill="Frequency", title="AA frequencies per position in Top10 nucleators")+
  theme_minimal()+
  theme(
    plot.margin = ggplot2::unit(c(1, 1, 1, 1), "points"))
...
ggsave(freq_heatmapTop10, file="AA_frequencyTop10_normall.jpg", width = 14, height = 8, path=path)
...
p_scatterTop10<-ggplot(coTop10, aes(x=factor(Var2, labels=23:34), y=value, group=1))+
  ...
  ylim(c(-0.25, 0.25))+
  facet_wrap(~factor(Var1, levels=vectorAA))+
  labs(x="Position", y="Frequency", title="Top10 nucleators (Normalized)")+
  theme_bw()+
  theme(
    plot.margin = ggplot2::unit(c(1, 1, 1, 1), "points"),
    legend.position = c(0.85, 0.2),
...
ggsave(p_scatterTop10, file="AA_frequencyTop10_posnormall.jpg", width = 14, height = 8, path=path)
...
nonnucleators_nucleators$pos_12<-sapply(strsplit(nonnucleators_nucleators$aa_seq_cor, ""), '[', 12)
...
aa_frequencies_pos_nonnucleators_<-rbind(pos_1, pos_2, pos_3, pos_4, pos_5, pos_6,
                                         pos_7, pos_8, pos_9, pos_10, pos_11, pos_12)
...
freq_heatmap_nonnucleators<-ggplot(co_nonnucleators, aes(factor(Var2, labels=(23:34)), factor(Var1, levels=rev(vectorAA))))+
  geom_tile(aes(fill=value), color="white", size=1)+
...
#
p_scatter_nonnucleators<-ggplot(co_nonnucleators, aes(x=factor(Var2, labels=23:34), y=value, group=1))+
...
diff_heatmap_nucleators_nonnucleators<-ggplot(co_nucleators_nonnucleators, aes(factor(Var2, labels=(23:34)), factor(Var1, levels=rev(vectorAA))))+
  geom_tile(aes(fill=diff), color="white", size=1)+
  scale_fill_gradient2(high="darkblue", mid= "white", low="darkred", limit=c(-0.08, 0.08))+
...
p_violin_p<-ggplot(co_nucleators_nonnucleators, aes(x=factor(Var2, labels=(23:34)), y=diff))+
...
co_nucleators_nonnucleators$pvalue<-""
co_nucleators_nonnucleators$Cohens_d<-""
for (aa in vectorAA){
  for (pos_ in 1:12){
    ...
    co_nucleators_nonnucleators[co_nucleators_nonnucleators$Var1 == aa &
                                  co_nucleators_nonnucleators$Var2 == paste0("pos_", pos_) &
                                  co_nucleators_nonnucleators$group == "nucleators",
                                "pvalue"] <- test_position_aa(pos_, aa)
    co_nucleators_nonnucleators[co_nucleators_nonnucleators$Var1 == aa &
                                  co_nucleators_nonnucleators$Var2 == paste0("pos_", pos_) &
                                  co_nucleators_nonnucleators$group == "nucleators",
                                "Cohens_d"] <- esc_chisq(p = test_position_aa(pos_, aa), totaln = table(nucleators_nucleators[paste0("pos_", pos_)])[aa]+table(nonnucleators_nucleators[paste0("pos_", pos_)])[aa])[[1]]
    
  }
}

nucleators_df<-co_nucleators_nonnucleators[co_nucleators_nonnucleators$group == "nucleators",] 
nucleators_df<-rename(nucleators_df, freq_nucleators = freq)
nonnucleators_df<-co_nucleators_nonnucleators[co_nucleators_nonnucleators$group == "Non-nucleators", c("ID", "freq")] 
nonnucleators_df<-rename(nonnucleators_df, freq_nonnucleators = freq)
df_<-merge(nucleators_df, nonnucleators_df, by="ID")
df_<-separate_wider_delim(df_, Var2, "_", names=c("P", "Pos"))
df_$Pos<-as.numeric(df_$Pos)+22
df_$ID<-paste0(df_$Var1, as.character(df_$Pos))
df_$Difference<-df_$freq_nucleators-df_$freq_nonnucleators
df_<-df_[c("ID", "freq_nucleators", "freq_nonnucleators", "pvalue", "Cohens_d", "Difference")]

write_tsv(df_, "Supplementary Table 3.tsv")


p_scatter_nucleators_nonnucleators<-ggplot(co_nucleators_nonnucleators, aes(x=factor(Var2, labels=23:34), y=freq ,group=group, color=group))+
  geom_hline(yintercept = 0, linewidth=.25)+
  geom_point(size=2)+
  geom_text(aes(label=significance), size=5, color="black", vjust=0.65, angle=90, y=0.0365)+
  geom_line(size=1)+
...
co_nucleators_nonnucleators_sum<-distinct(co_nucleators_nonnucleators_sum[c("Var2", "sum_freq", "group", "type")])
...
for (type_ in unique(AA_type$type)){
  for (pos_ in 1:12){
    ...
    co_nucleators_nonnucleators_sum[co_nucleators_nonnucleators_sum$type == type_ &
                                      co_nucleators_nonnucleators_sum$Var2 == paste0("pos_", pos_) &
                                      co_nucleators_nonnucleators_sum$group == "nucleators",
                                    "pvalue"] <- test_position_types(pos_, type_)
    co_nucleators_nonnucleators_sum[co_nucleators_nonnucleators_sum$type == type_ &
                                      co_nucleators_nonnucleators_sum$Var2 == paste0("pos_", pos_) &
                                      co_nucleators_nonnucleators_sum$group == "nucleators",
                                    "Cohens_d"] <- esc_chisq(p = test_position_types(pos_, type_), 
                                                             totaln = sum(table(nucleators_nucleators[paste0("pos_", pos_)])[AA_type[AA_type$type == type_, "AA"]])+sum(table(nonnucleators_nucleators[paste0("pos_", pos_)])[AA_type[AA_type$type == type_, "AA"]]))[[1]]
    
  }
}

co_nucleators_nonnucleators_sum<-separate_wider_delim(co_nucleators_nonnucleators_sum, Var2, "_", names=c("P", "Pos"))
co_nucleators_nonnucleators_sum$Pos<-as.numeric(co_nucleators_nonnucleators_sum$Pos)+22
co_nucleators_nonnucleators_sum$ID<-paste0(co_nucleators_nonnucleators_sum$type, "_", as.character(co_nucleators_nonnucleators_sum$Pos))

nucleators_df<-co_nucleators_nonnucleators_sum[co_nucleators_nonnucleators_sum$group == "nucleators",] 
nucleators_df<-rename(nucleators_df, freq_nucleators = sum_freq)
nonnucleators_df<-co_nucleators_nonnucleators_sum[co_nucleators_nonnucleators_sum$group == "Non-nucleators", c("ID", "sum_freq")] 
nonnucleators_df<-rename(nonnucleators_df, freq_nonnucleators = sum_freq)
df_<-merge(nucleators_df, nonnucleators_df, by="ID")
df_$Difference<-df_$freq_nucleators-df_$freq_nonnucleators
df_<-df_[c("ID", "freq_nucleators", "freq_nonnucleators", "pvalue", "Cohens_d", "Difference")]

write_tsv(df_, "Supplementary Table 4.tsv")



p_scatter_nucleators_nonnucleators_types<-ggplot(co_nucleators_nonnucleators_sum, aes(x=factor(Pos, labels=23:34), y=sum_freq ,group=group, color=group))+
  geom_hline(yintercept = 0, linewidth=.25)+
  geom_point(size=2)+
  geom_line(size=1)+
  geom_text(aes(label=significance), size=5, color="black", vjust=0.65, angle=90, y=0.05)+
  scale_color_manual(values=c("darkred", "darkblue"), label=c("non-nucleator", "nucleator"))+
  ylim(c(-0.07, 0.07))+
